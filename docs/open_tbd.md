...
- [ ] **Real production concurrency numbers** for all 6 backends — current values
  are first-draft placeholders. Ivo confirmed (2026-07-29): keep as-is for now;
  a dedicated load-testing effort is planned as future work to establish real
  numbers (see `implementation_plan.md` Phase 6, §7).
- [ ] **Real backend upstream hosts/ports** — pywb ports (8080–8083) and the
  ...
  depends on which server/cluster the AAC is deployed against, so values must
  be set per environment rather than resolved once here.
...
- [ ] **GeoIP/ASN `db_path` and refresh mechanism** — placeholder path
  ...
  is implemented.
- [ ] **`ingress.trusted_proxies`** — placeholder `["127.0.0.1"]`
  (`implementation_plan.md` §2.3); must be set to the real front-end Apache
  httpd/Caddy IP(s)/CIDRs before deployment (FR-010a). *(pre-existing, not
  previously tracked here — added 2026-07-29 for completeness.)*
